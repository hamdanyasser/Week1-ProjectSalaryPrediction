...
A personal project that trains a Decision Tree model on the Kaggle DS Salaries dataset, serves predictions through a GET API, analyzes them with a local LLM, persists results to Supabase, and presents everything in a Streamlit dashboard that a non-technical audience can follow.
...
A separate Python script calls the API with proper error handling.
...
The dashboard shows narrative insights and a supporting visualization generated by a local LLM.
...
Predictions are persisted to Supabase and read back in the dashboard.
